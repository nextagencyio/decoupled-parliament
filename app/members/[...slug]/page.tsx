...
import { GET_MEMBER_BY_PATH } from '@/lib/queries'
...
interface MemberByPathData {
  route: {
    entity: DrupalMember
  } | null
}

async function getMember(path: string): Promise<DrupalMember | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<MemberByPathData>({
      query: GET_MEMBER_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getMember(path)

  if (!item) {
    return { title: 'Member Not Found | Parliament of Aldoria' }
  }
                ...
                <dl className="space-y-4">
                  {(item as any).constituency && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).constituency}</dd>
                    </div>
                  )}
                  ...
                  {(item as any).firstElected && (
                    <div>
                      <dt className="text-sm text-gray-500">First Elected</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).firstElected}</dd>
                    </div>
                  )}
                  {(item as any).email && (
                    <div>
