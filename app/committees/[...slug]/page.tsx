...
import { GET_COMMITTEE_BY_PATH } from '@/lib/queries'
import { DrupalCommittee } from '@/lib/types'
...
interface CommitteeByPathData {
  route: {
    entity: DrupalCommittee
  } | null
}

async function getCommittee(path: string): Promise<DrupalCommittee | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<CommitteeByPathData>({
      query: GET_COMMITTEE_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/committees/${slug.join('/')}`
  const item = await getCommittee(path)

  if (!item) {
    return { title: 'Committee Not Found | Parliament of Aldoria' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).chairName && (
                    <div>
                      <dt className="text-sm text-gray-500">Committee Chair</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).chairName}</dd>
                    </div>
                  )}
                  {(item as any).meetingSchedule && (
                    <div>
                      <dt className="text-sm text-gray-500">Meeting Schedule</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).meetingSchedule}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
